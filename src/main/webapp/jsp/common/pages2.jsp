...
<div class="container">
	<ul class="pagination">
		<li><a href="javascript:sendParams2(1)">首页</a></li>
		<c:if test="${page.pageNow>1}">
			<li><a href="javascript:sendParams2(${page.pageNow-1})">&laquo;</a></li>
		</c:if>
		...
		<c:forEach begin="${pagebegin}" end="${pageend}" step="1" var="p">
			<li <c:if test="${page.pageNow==p}">class="active"</c:if>><a
				href="javascript:sendParams2(${p})">${p}</a></li>
		</c:forEach>

		<c:if test="${page.pageNow<page.pageCount}">
			<li><a href="javascript:sendParams2(${page.pageNow+1})">&raquo;</a></li>
		</c:if>
		<li><a href="javascript:sendParams2(${page.pageCount})">尾页</a></li>
		...
		<li><a style="padding: 0px; border: 0px"> <span>每页显示</span> <select
				...
				id="pageSize" onchange="sendParams2(1)">
